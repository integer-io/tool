...
export const ImageEditor = ({ user, onAuthRequired }: ImageEditorProps) => {
  ...
  const removeBgApiKey = getApiKey('removebg') || '';
  ...
  const removeBackground = async () => {
    if (!handleAuthCheck() || !selectedImage) return;
    ...
    <div className="space-y-8">
      <Card className="bg-gradient-card backdrop-blur-lg border-white/20 shadow-card">
        <CardHeader>
          ...
          <CardDescription>
            Edit images with filters, transformations, and AI-powered background removal
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <Input
              id="removebg-api-key"
              type="password"
              placeholder={hasApiKey('removebg') ? "API key saved (click to change)" : "Enter your Remove.bg API key"}
              value={removeBgApiKey}
              onChange={(e) => setApiKey('removebg', e.target.value)}
                ...
                <TabsContent value="ai" className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <Button
                      ...
                      variant="studio"
                      disabled={!user || isProcessing || !removeBgApiKey.trim()}
                    ...
                    >
                      <div className="text-center">
                        <Crop className="h-6 w-6 mx-auto mb-2" />
                        <div className="font-semibold">Crop Square</div>
                        <div className="text-xs opacity-75">
